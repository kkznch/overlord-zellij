...
新規作成前にセッションが既に存在するか確認しなければならない (SHALL)。`zellij list-sessions` の出力でセッション名を検索する際、部分一致ではなく完全一致で判定しなければならない (SHALL)。

#### Scenario: 既存セッションの確認
- **WHEN** セッションの存在を確認する時
- **THEN** `zellij list-sessions` を実行し、セッション名を出力から完全一致で検索する
