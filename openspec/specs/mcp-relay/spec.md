...
MCP サーバーは `send_message` ツールを公開し、受信者の inbox にメッセージを保存して通知をトリガーしなければならない (SHALL)。
...
MCP サーバーは `OVLD_ROLE`、`OVLD_RELAY_DIR`、`OVLD_SESSION` 環境変数を読み取って自身を識別し、メッセージストアの場所を特定しなければならない (SHALL)。
